...
@interface DoTeacherController ()<JZLCycleViewDelegate>
...
@property(nonatomic,strong) TeacherBottomView *teacherVottomView;
...
@implementation DoTeacherController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(topicNotice) name:@"teacherTopic" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(orderNotice) name:@"teacherOrder" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(wordNotice) name:@"teacherWord" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(incomeNotice) name:@"teacherincome" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(homeNotice) name:@"teacherhome" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setHomeNotice) name:@"teacherSetHome" object:nil];
...
- (void)addContenView
{
    _teacherVottomView = [[[NSBundle mainBundle] loadNibNamed:@"TeacherBottomView" owner:nil options:nil] lastObject];
    _teacherVottomView.frame = CGRectMake(0, _middView.sh_bottom, ScreenWidth, 180);
    [self.scrollView addSubview:_teacherVottomView];
}
...
- (void)addBtn
{
    ...
    _btn.frame = CGRectMake(ScreenWidth * 0.1,_teacherVottomView.sh_bottom + 50,ScreenWidth * 0.8,30);
