...
@interface DoMasterController ()<JZLCycleViewDelegate>
...
@property(nonatomic,strong) MasterBottomView *masterBottomView;
...
@implementation DoMasterController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(topicNotice) name:@"masterTopic" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(orderNotice) name:@"masterOrder" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(wordNotice) name:@"masterWord" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(incomeNotice) name:@"masterincome" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(homeNotice) name:@"masterhome" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setHomeNotice) name:@"masterSetHome" object:nil];
...
- (void)addContenView
{
    _masterBottomView = [[[NSBundle mainBundle] loadNibNamed:@"MasterBottomView" owner:nil options:nil] lastObject];
    _masterBottomView.frame = CGRectMake(0, _middView.sh_bottom, ScreenWidth, 180);
    [self.scrollView addSubview:_masterBottomView];
}
...
- (void)addBtn
{
    ...
    _btn.frame = CGRectMake(ScreenWidth * 0.1,_masterBottomView.sh_bottom + 50,ScreenWidth * 0.8,30);
